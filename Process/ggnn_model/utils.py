...
def to_tensor_func(array, device):
    return torch.tensor(array).to(device).float()  # double -> flaot
...
def preprocess_adj(adj):
    ...
    return np.array(list(adj))  # coo_to_tuple(sparse.COO(np.array(list(adj)))), mask
