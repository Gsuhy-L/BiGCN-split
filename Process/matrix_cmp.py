...
def get_matrix(dataset_name):
    ...
    for i in range(len(rvnn_files)):
        ...
        # if '519868410599993344' not in rvnn_file:
        #     continue
        rvnn_num, rvnn_matrix, rvnn_root = rvnn_file_data['num'],rvnn_file_data['edgeindex'],rvnn_file_data['rootindex']
        bigcn_num, bigcn_matrix, bigcn_root, bigcn_x, bigcn_y = bigcn_file_data['num'], bigcn_file_data['edgeindex'], \
                                                                bigcn_file_data['rootindex'],bigcn_file_data['x'],bigcn_file_data['y']
        #接下来先尝试把矩阵构建出来
        # print(rvnn_num)
        # print(rvnn_matrix)
        # print(bigcn_num)
        ...
        # print(bigcn_matrix)
            ...
            #     print('key_1',rvnn_new_map[1])
                # rvnn_new_map[key].sort(key=lambda x: float(x[1]))
                            # new_map
            # print(rvnn_new_map)
            # print('rvnn_key_1',rvnn_new_map[1])
            ...
            #     if key == 2:
            #         print('bigcn_len_key',key,val)
            #
            # for key, val in rvnn_new_map.items():
            #     if key == 2:
            #
            #         print('rvnn_nen_key', key, val)
            ...
            for key,val in bigcn_new_map.items():
                # if key == 2:
                #     print('------------------------------------==========')
                #     print(rvnn_new_map[1])
                #     print(bigcn_new_map[1])

                for i in bigcn_new_map[key]:
                    # print(i)
                    flag = 0
                    len_bigcn_time_key = len(bigcn_time_map[key])
                    #查找对应的时间
                    #rvnn 下标 时间 父节点
                    bigcn_node, bigcn_parent = i[0],i[1]

                    #bigcn_node
                    # [81, 55], [27, 55], [8, 55], [77, 53],
                    # print()
                    # print('====================================')
                    # print(len(rvnn_new_map[key]))
                    for j in range(len(rvnn_new_map[key])):
                    #     print(j)
                        # print('--------------')
                        # print(i)
                        # print(key)
                        # print(j,len(rvnn_new_map[key]))
                        sort_key_new_rvnn_map = sorted(rvnn_new_map[key], key=lambda x: float(x[1]))

                        if key == 1:
                            first_child_time = sort_key_new_rvnn_map[0][1]
                        # print(sort_key_new_rvnn_map)
                        # print(rvnn_new_map[key])
                        # print(sort_key_new_rvnn_map)
                        # print(e)
                        # parent_rvnn_data = sort_key_new_rvnn_map[len(rvnn_new_map[key])-j-1]
                        parent_rvnn_data = sort_key_new_rvnn_map[j]

                    # print(rvnn_new_map[key])
                        # print(parent_rvnn_data)
                        rvnn_node,rvnn_time,rvnn_parent = parent_rvnn_data[0],parent_rvnn_data[1],parent_rvnn_data[2]
                        ...
                        # print(e)
                            ...
                            #     print(key)
                            #     print(i)
                            #     # print(bigcn_time_map[key-2])
                            #     print(bigcn_time_map[key-1])
                            #     print(bigcn_new_map[key-1])
                            #     print(e)
                            # if not parent_flag:
                            #     print(bigcn_time_map)
                            #     print(key)
                            ...
                            if float(rvnn_time) <= float(bigcn_time_parent_time):
                                # break
                                # print('min-----------')
                                # print(bigcn_time_parent_time)
                                # print(rvnn_time)
                                continue
                            else:
                                flag=1
                            #如果rvnn大于父节点时间
                                ...
                                select_rvnn_node.append(rvnn_node)
                                break
                    # if len(bigcn_time_map[key]) == len_bigcn_time_key:
                    #     print(bigcn_time_map[key])
                    #     print(key)
                    #     print(e)
                    # print(flag)
                    if not flag:
                        # print(e)
                            # flag = 1
                        bigcn_time_map[key].append([bigcn_node, str(float(first_child_time)+float(key*60)), bigcn_parent])
                        # print(j)
                    # if not flag:
                    #     print(e)
                        # if float(j[1])
            ...
            #现在有了数据要往回写数据
            #        new_rvnn_time = np.insert(rvnn_matrix[2],0,'0.0')
            # print('select_rvnn:',len(select_rvnn_node))

            # for key,val in bigcn_time_map.items():
            #     print(len(val))
            #     print('----------------')
            #     print(rvnn_new_map[key])
            #     print(bigcn_time_map[key])
        ...
        np.savez(os.path.join(bigcn_path,  dataset_name + 'fulltime/' + str(id) + '.txt'),  edgeindex=bigcn_matrix, edgetime = new_bigcn_time,
                 rootindex=bigcn_root, x = bigcn_x, y = bigcn_y)
...
print(get_matrix(dataset_name='Twitter16'))
