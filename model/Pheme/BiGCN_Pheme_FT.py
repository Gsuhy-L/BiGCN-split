...
class Net(th.nn.Module):
    def __init__(self,in_feats,hid_feats,out_feats):
        ...
        self.fc=th.nn.Linear((out_feats)*4,4)
    ...
    def forward(self, data):
        ...
        return x
def train_GCN(treeDic, x_test, x_train,TDdroprate,BUdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter,split_num):
    ...
    for epoch in range(n_epochs):
        ...
        traindata_list, testdata_list = loadBisplitFTData(dataname, treeDic, x_train, x_test, TDdroprate, BUdroprate,
                                                          split_num=split_num)
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_Acc_all), np.mean(temp_val_Acc1),
                       ...
                       np.mean(temp_val_F1),
                       np.mean(temp_val_F2), model, 'BiGCN', dataname)
...
def run_main(split_num):
    def set_seed(seed=1):
        ...
        print("seed:", seed)
    set_seed(123)
    f = open('/home/ubuntu/PyProjects_gsuhyl/PyProjects/BiGCN-master/'+'Process/count_split/'+datasetname+'res.txt','a')
    # print(eee)
    ...
    res = "Total_Test_ Accuracy: {:.4f}|acc1: {:.4f}|acc2: {:.4f}|pre1: {:.4f}|pre2: {:.4f}""|rec1: {:.4f}|rec2: {:.4f}|F1: {:.4f}|F2: {:.4f}".format(sum(test_accs) / iterations, sum(ACC1) / iterations,
    ...
    print(res)
    f.write(split_num+res+"\n")
    f.close()


for i in range(0,101,5):
    run_main(split_num=str(i))